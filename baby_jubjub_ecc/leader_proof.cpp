    ...
    gadget<FieldT>(pb, annotation_prefix)
{
    ...
    randomCommit.reset(new pedersen_hash<FieldT>(pb, FMT(annotation_prefix, "random number")));
...
{
    this -> pb.val(T) = this -> pb.val(in_T);
    this -> pb.val(block_hash) = in_block_hash;
    this -> pb.val(sl) = in_sl;
    this -> pb.val(sn_x) = this-> pb.val(sn_commit_x);
    this -> pb.val(sn_y) = this-> pb.val(sn_commit_y);
    this ->sn_m.fill_with_bits_of_field_element(this-> pb, in_sn_m);
    this ->sn_r.fill_with_bits_of_field_element(this-> pb, in_sn_r);

    this -> pb.val(rep_x) = this-> pb.val(rep_commit_x);
    this -> pb.val(rep_y) = this-> pb.val(rep_commit_y);
    this ->rep_m.fill_with_bits_of_field_element(this-> pb, in_rep_m);
    this ->rep_r.fill_with_bits_of_field_element(this-> pb, in_rep_r);

    this -> pb.val(rep) = FieldT(rep_m);
    ...
    this->pb.val(ran) = randomCommit -> get_res_x();
